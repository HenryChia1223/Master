...
namespace SabberStoneCoreConsole
{
	internal class Program
	{
		...
		static void Main(string[] args)
		{
			...
			//Console.WriteLine("Start Test!");//刪除註解

			//Console.WriteLine(Cards.Statistics());//刪除註解
			...
			//Console.WriteLine("Finished! Press key now.");//刪除註解
			Console.ReadKey();
		}
		...
		static void EntityChangeTest()
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				game.Process(EndTurnTask.Any(game.CurrentPlayer));
				//Console.WriteLine($"{molten.Card.Name}");//刪除註解
			}
			...
			{
				//Console.WriteLine("Something goes wrong!");//刪除註解
			}
		}
		...
		public static void QuestDrawFirstTest()
		{
			...
			//Console.WriteLine(game.CurrentOpponent.HandZone.FullPrint());//刪除註解
			//Console.WriteLine(game.CurrentOpponent.BoardZone.FullPrint());//刪除註解
		}
			...
			for (int i = 0; i < total; i++)
			{
				...
				while (game.State != State.COMPLETE)
				{
					...
					if (loopCount > 10)
					{
						...
						Console.WriteLine("");
						//Console.WriteLine(game.FullPrint());//刪除註解
						Console.WriteLine("");
						options.ForEach(p =>
						{
							//Console.WriteLine(p.FullPrint());//刪除註解
						});
		...
		private static void ChooseOneTest()
		{
			...
			//options.ForEach(p => Console.WriteLine(p.FullPrint()));//刪除註解
		...
		private static void PowerHistoryTest()
		{
			...
			{
				List<PlayerTask> options = game.CurrentPlayer.Options();
				//Console.WriteLine($" *** - {game.CurrentPlayer.Name} options on {game.Turn}. - ***");//刪除註解
				//options.ForEach(p => Console.WriteLine(p.FullPrint()));//刪除註解

				//Console.WriteLine(PowerOptionsBuilder.AllOptions(game, options)?.Print());//刪除註解
		...
		public static void GameSplitTest()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, "Loot Hoarder"));
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			//Console.WriteLine(game.CurrentOpponent.DeckZone.Count);//刪除註解
		...
		public static void GameMulliganTest()
		{
			...
			//Console.WriteLine(game.Player1.Choice?.FullPrint());//刪除註解
			//Console.WriteLine(game.Player2.Choice?.FullPrint());//刪除註解
		}
		...
		public static void CloneStampTest()
		{
			...
			//Console.WriteLine($"GameState: {game.State}");//刪除註解
