...
namespace GeneticProgramming
{
	class Program
	{
		...
		public static int population_size = 30;//人口數   //3 2000*2000 300M//
		public static int generation_num = 30;//世代數
		...
		static void Main(string[] args)
		{
			...
			for (int g = 0; g < generation_num; g++)//菁英法
			{
				...
				else //交配
				{
					...
					//else
					//{
					//	string a = best_tree_in_ger[0].rebuild_pre(best_tree_in_ger[0]);
					//	best_tree_in_ger[0].preorder_path = "";

					//	cross2_tree = reb.ToList();//每代的best放進cross1
					//	reb.Clear();
					//}
				...
				Console.WriteLine("best_tree postorder:" + s);

				s= best_tree[0].InOrder(best_tree[0]);
				best_tree[0].inorder_path = "";
				Console.WriteLine("best_tree inorder:" + s);
				...
				s = best_tree_in_ger[0].PostOrder(best_tree_in_ger[0]);
		...
		public static void fitness(List<Node> tree)//算勝率，存入字典winrate
		{
			...
			string find = tree[0].rebuild_pre(tree[0]);
			...
			if (dic_score_preorder.ContainsKey(find))//若查到
			{
				...
				SabberStoneCoreAi.src.Program.main.main_start(s);

				Console.WriteLine("算完了");
				Console.WriteLine("抓到的勝率為playA:" + SabberStoneCoreAi.POGame.GameStats.return_winrate);
				Console.WriteLine();
